...
RSpec.describe Classifier::NaiveBayes do
  ...
  context "with 4 Field Of Research codes and topic data from theconversation.com" do
    ...
    before do
      ...
      classifier.train(:maths, "Mathematics Learning Early childhood education Parents Early childhood learning Parents role in education")
    ...
    context "with a sport result input" do
      let(:result) { classifier.classify("psychoanalysis") }
      ...
      it "calculates correct scores" do
        ...
        expect(result.scores[:maths].round(10)).to eq(BigDecimal.new("0.0014204545"))
        expect(result.scores[:medical].round(10)).to eq(BigDecimal.new("0.0036607688"))
        expect(result.scores[:education].round(10)).to eq(BigDecimal.new("0.002050581"))
        expect(result.scores[:law].round(10)).to eq(BigDecimal.new("0.0020053476"))
      end
    end
  end
end
